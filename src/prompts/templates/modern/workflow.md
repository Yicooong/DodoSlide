...
### 配色约束（Modern Academic Minimalism）：

- 使用**1个主色 + 1–2个辅助色 + 中性色（黑/白/灰）**的极简配色结构
- 整体以中性色为主（≥80%），强调色仅用于关键信息（≤20%）
- 保持高对比度（文本与背景清晰可读）
- 主色需全局一致，用于标题、关键数据与视觉锚点
- 采用低饱和度色彩，避免高纯度颜色造成视觉干扰
- 建立颜色层级：主信息（高对比/主色）> 次信息（灰度）> 辅助信息（弱化色）
- 避免多强调色并行，单页仅允许一个视觉焦点
- 图表颜色需具备语义一致性（同类同色，对比用对比色，避免彩虹配色）
...
| 维度 | 规则说明 |
|------|----------|
| **色彩比例** | **6-3-1 原则**：60% 背景  / 30% 骨架  / 10% 强调  |
