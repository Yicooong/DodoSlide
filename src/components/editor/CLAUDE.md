...
**功能特性:**
- **Monaco Editor**: 使用 VS Code 的编辑器核心（`@monaco-editor/react`）
- **实时编辑**: 代码变化实时传递给父组件
- **主题支持**: 支持深色和浅色主题切换（vs-dark / vs）
- **自动布局**: 编辑器高度随容器自适应（`automaticLayout: true`）
...
4. **自动布局**: `automaticLayout: true` 避免手动计算大小
...
5. **编辑器挂载**: 使用 `onMount` 回调访问 Monaco 实例进行高级配置
